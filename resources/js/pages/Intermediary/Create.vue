                ...
                <ButtonWhite
                    ...
                    @click="
                        $vfm.show('redirect-modal', {
                            title: 'Cancel intermediary Registration?',
                            description: 'When you abort, all data is discarded. Are you sure you really want to cancel this intermediary registration?',
                        })
                    "
                >
                    {{ $t("Cancel") }}
  ...
  <script setup>
    ...
    const submit = async () => {
        ...
        try {
            ...
            const res = await form.post(route("intermediaries.store"));
            router.push({ name: "intermediary-index" });

            const message = form.send_mail ? 'The intermediary was successfully created and the Email invitation sent' : 'The intermediary was successfully created'
            notificationForSuccess.success((message), trans("Notification"));
        } catch (error) {
    ...
    if( !permission.hasPermission('intermediares.create')){
